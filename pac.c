...
struct __attribute__((packed, aligned(2))) package{
   ...
   unsigned char game_st, pp, pu, pu_a, pu_c;
...
char maze[LABY_HEIGHT][LABY_WIDTH] = {                                                                                 //x
	...
	{'|',' ',' ',' ',' ',' ',' ','|','|',' ',' ',' ',' ','|','|',' ',' ',' ',' ','|','|',' ',' ',' ',' ',' ',' ','|'}, //9
	{'[','-','-','-','-','`',' ','|','[','-','-','`',' ','|','|',' ','.','-','-',']','|',' ','.','-','-','-','-',']'}, //10
	{'x','x','x','x','x','|',' ','|','.','-','-',']','w','[',']','w','[','-','-','`','|',' ','|','x','x','x','x','x'}, //11
	{'x','x','x','x','x','|',' ','|','|','w','w','w','w','w','w','w','w','w','w','|','|',' ','|','x','x','x','x','x'}, //12
	{'x','x','x','x','x','|',' ','|','|','w','.','-','t','f','f','r','-','`','w','|','|',' ','|','x','x','x','x','x'}, //13
	{'q','-','-','-','-',']',' ','[',']','w','|','x','x','w','w','x','x','|','w','[',']',' ','[','-','-','-','-','y'}, //14
	{'o','w','w','w','w','w',' ','w','w','w','|','x','x','w','w','x','x','|','w','w','w',' ','w','w','w','w','w','i'}, //15
	{'r','-','-','-','-','`',' ','.','`','w','|','x','x','w','w','x','x','|','w','.','`',' ','.','-','-','-','-','t'}, //16
	{'x','x','x','x','x','|',' ','|','|','w','[','-','-','-','-','-','-',']','w','|','|',' ','|','x','x','x','x','x'}, //17
	{'x','x','x','x','x','|',' ','|','|','w','w','w','w','w','w','w','w','w','w','|','|',' ','|','x','x','x','x','x'}, //18
	{'x','x','x','x','x','|',' ','|','|','w','.','-','-','-','-','-','-','`','w','|','|',' ','|','x','x','x','x','x'}, //19
	{'.','-','-','-','-',']',' ','[',']','w','[','-','-','`','.','-','-',']','w','[',']',' ','[','-','-','-','-','`'}, //20
	{'|',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ','|','|',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ','|'}, //21
	{'|',' ','.','-','-','`',' ','.','-','-','-','`',' ','|','|',' ','.','-','-','-','`',' ','.','-','-','`',' ','|'}, //22
	{'|',' ','[','-','`','|',' ','[','-','-','-',']',' ','[',']',' ','[','-','-','-',']',' ','|','.','-',']',' ','|'}, //23
...
int client () {
	...
	bcopy((char*)ip->h_addr,(char*)&serv_addr.sin_addr.s_addr,ip->h_length);
...
void *server_fantasma(void *sn){
	...
	while(paquete.game_st == 0 && server == 1){
		...
		if (a != jugador) {n = read(newsockfd,&paquete.jugadores[a].keyst,sizeof(char));
            if(n < 0){
			errco = 4;
            paquete.jugadores[a].sel = 0;
			return NULL;
		}
		paquete.jugadores[a].sel = 1;
		control(a);
        };
...
if (paquete.jugadores[i].death != 1) {
     if (paquete.pu_a == 3 && i != 0) {
         switch (paquete.jugadores[i].keyst){
      case('B'): if (!collition(i,dir[i] = 2)) {paquete.jugadores[i].ax = 0; paquete.jugadores[i].ay = -1;} break;
      case('A'): if (!collition(i,dir[i] = 1)) {paquete.jugadores[i].ax = 0; paquete.jugadores[i].ay = 1;} break;
      case('D'): if (!collition(i,dir[i] = 3)) {paquete.jugadores[i].ax = 1; paquete.jugadores[i].ay = 0;} break;
      case('C'): if (!collition(i,dir[i] = 4)) {paquete.jugadores[i].ax = -1; paquete.jugadores[i].ay = 0;} break;
      case('p'): if (server == 1 && i != jugador) {paquete.jugadores[i].sel = 0; paquete.jugadores[i].keyst = 'r';} break;
      case(' '): if (paquete.pu != 0 && jugador == 0) {paquete.pu_a = paquete.pu; paquete.pu = 0;}; break;
      default: break;
    }
     }
	...
	switch (paquete.jugadores[i].keyst){
      ...
      case('q'): if (server == 1 && i != jugador) {paquete.jugadores[i].sel = 0; paquete.jugadores[i].keyst = 'r';} break;
      case(' '): if (paquete.pu != 0 && jugador == 0) {paquete.pu_a = paquete.pu; paquete.pu = 0;}; break;
      default: break;
    }
    }
...
void print_pac(WINDOW *screen, int status) {
	...
	if (paquete.pu_a != 2) wattron(screen, COLOR_PAIR(2));
	else wattron(screen, COLOR_PAIR(1));
	int set = pla_dir(0);
    
    if (paquete.jugadores[0].death || (paquete.pu_a == 2 && jugador != 0)) {mvwprintw(screen, y_in_laby(paquete.jugadores[0].y), x_in_laby(screen, paquete.jugadores[0].x), " " );}
    else {
...
void collition_pac (WINDOW *screen) {
    ...
    if (paquete.cur_maze[paquete.jugadores[0].y][paquete.jugadores[0].x] == 0) {paquete.cur_maze[paquete.jugadores[0].y][paquete.jugadores[0].x] = -2; paquete.pills--; if (paquete.pu == 0 && paquete.pu_a == 0) {paquete.pu_c++;}; };
...
void power_manager() {
    static unsigned end;
     ...
     switch (paquete.pu_a) {
         default:
         case 0: break;
         case 1: if (paquete.jugadores[0].x < 13) {paquete.jugadores[0].x = LABY_WIDTH - 2 - paquete.jugadores[0].x;} else {paquete.jugadores[0].x = (paquete.jugadores[0].x - LABY_WIDTH)*-1;}; paquete.pu_a = 0; paquete.pu = 0; paquete.pu_c = 0; break;
         case 2: 
         case 3: if (end == 0) {end = paquete.msec/1000 + 10;} if (end == paquete.msec/1000) {paquete.pu_a = 0; paquete.pu = 0; paquete.pu_c = 0; end = 0;}; break;
    }
}
...
signed char menu (WINDOW *screen, int argc) {
    ...
    mvwprintw (screen, y_in_laby(0), x_in_laby(screen, 5), "Pᗧc Mᗣn");
     ...
     if (argc < 2) { mvwprintw (screen, y_in_laby(4), x_in_laby(screen, 3), "Unirse a Partida (No disponible)");}
     mvwprintw (screen, y_in_laby(6), x_in_laby(screen, 3), "Instrucciones");
     mvwprintw (screen, y_in_laby(8), x_in_laby(screen, 3), "Salir");
     
     if (argc < 2) { mvwprintw (screen, y_in_laby(14), x_in_laby(screen, 3), "Una ip no fue puesta en parametros.");
         mvwprintw (screen, y_in_laby(15), x_in_laby(screen, 3), "Modo cliente no estara disponible.");
    }
...
int sel_menu (WINDOW *screen) {
    ...
    if (cursor != 5) {jugador = cursor;}
     ...
     if (w_cursor == '\r') {
        if (cursor == 5) {jugador = -2; break;}
        else {
            if (server == 1) {break;}
            else if (server == 0) { 
                    client();
                    if (paquete.jugadores[jugador].sel == 1) { mvwprintw (screen, y_in_laby(14), x_in_laby(screen, 0), "Personaje ya seleccionado o error de conexion");wrefresh(screen);sleep(2);jugador = -1;}
                    else {break;}
            }
        }
    }
...
void tutorial(WINDOW *screen){
    ...
    mvwprintw (screen, y_in_laby(2), x_in_laby(screen, 3), "Intrucciones");
    
    mvwprintw (screen, y_in_laby(4), x_in_laby(screen, 0), "Crea una partida para iniciar un juego");
    mvwprintw (screen, y_in_laby(5), x_in_laby(screen, 0), "Unete a una partida para jugar en una ya empezada");
     mvwprintw (screen, y_in_laby(6), x_in_laby(screen, 0), "(Requiere introducir una ip como parametro)");
      ...
      mvwprintw (screen, y_in_laby(13), x_in_laby(screen, 0), "Usa las Power-pills (⦿) para devorar y frenar los fantasmas");
       ...
       mvwprintw (screen, y_in_laby(19), x_in_laby(screen, 2), "Curs: Inverte el movimiento de los fantasmas");
       
       mvwprintw (screen, y_in_laby(21), x_in_laby(screen, 0), "Presione cualquier tecla para volver al menu");
       wrefresh(screen);
       wgetch(screen);
}
...
int game_loop (WINDOW *w, WINDOW *field, WINDOW *score) {
  ...
  draw_borders(score);
    ...
    wrefresh(score);
    paquete.jugadores[jugador].keyst = 'r';
    keyst_local == 'r';
    
if (server == 1) {start = clock(); srand((int) start);}
...
while (paquete.game_st == 0) {
    ...
    mvwprintw(score, 1, 1, "Poder");
    mvwprintw(score, 1, 6, "[");
    mvwprintw(score, 1, 11, "]");
    
    if (paquete.pu_c > 1 && paquete.pu_c < 30) {mvwprintw(score, 1, 7, "L");};
    if (paquete.pu_c > 9 && paquete.pu_c < 30) {mvwprintw(score, 1, 8, "L");};
    if (paquete.pu_c > 19 && paquete.pu_c < 30) {mvwprintw(score, 1, 9, "L");};
	if (paquete.pu_c > 29) {
        if (paquete.pu == 0 && paquete.pu_a == 0) paquete.pu = 3;
        switch (paquete.pu) {
            case 1: mvwprintw(score, 1, 7, "Mirr"); break;
            case 2: mvwprintw(score, 1, 7, "Invi"); break;
            case 3: mvwprintw(score, 1, 7, "Curs"); break;
            default: break;
        }};
    ...
    if (paquete.jugadores[0].lives == 0) {
        paquete.jugadores[jugador].keyst = 'r';
        keyst_local == 'r';
        paquete.pu = 0;
        paquete.pu_c = 0;
        paquete.pu_a = 0;
        return 1;}
    if (paquete.jugadores[jugador].keyst == 'q' && server == 1) {
        paquete.jugadores[jugador].keyst = 'r';
        keyst_local == 'r';
        paquete.pu = 0;
        paquete.pu_c = 0;
        paquete.pu_a = 0;
        return 2;}
    if (keyst_local == 'q' && server == 0) {
        paquete.jugadores[jugador].keyst = 'q';
        keyst_local == 'r';
        paquete.pu = 0;
        paquete.pu_c = 0;
        paquete.pu_a = 0;
...
if (argc > 1) {ip = gethostbyname(argv[1]);};
...
paquete.jugadores[jugador].sel = 1;
